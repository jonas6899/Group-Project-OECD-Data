...
def wrap_text(text, width):      # wraps long legend titles
...
def display_data(facecolor, labelcolor, data, plottype, selectedcountry, selectedtimestart=2015, selectedtimeend=2023):
    ...
    for i, category in enumerate(categories):
        if plottype == "line":
            ...
            ax.scatter(data["TIME_PERIOD"], data[category], label=wrap_text(legend_mapping[category], 15), color=colors[i])
        elif plottype == "bar":
            ax.bar(data["TIME_PERIOD"] + i * 0.1, data[category], label=wrap_text(legend_mapping[category], 15), width=0.1,
                   color=colors[i])
        else:
...
if __name__ == "__main__":
    ...
    fig = display_data(facecolor, labelcolor, data, plottype, selectedcountry, selectedtimestart=2015,
